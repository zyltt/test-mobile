...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  // u32 first = hash32(first_trace, MAP_SIZE, HASH_CONST);
  // SAYF("%d\n", first);
  
  // u32 ck1 = hash32(trace_bits, MAP_SIZE, HASH_CONST);
  // SAYF("%d\n", ck1);

  // u32 ck2 = hash32(virgin_bits, MAP_SIZE, HASH_CONST);
  // SAYF("%d\n", ck2);
...
static void check_map_coverage(void) {
  ...
  SAYF(cGRA "****Check map coverage\n" cRST);
  // for(j = (1 << (MAP_SIZE_POW2 - 1)); j < MAP_SIZE; j++){
  //   SAYF("%d ", trace_bits[j]);
  // }
...
static void perform_dry_run(char** argv) {
  ...
  while (q) {
    ...
    //print out the file name here.

    if (res == crash_mode || res == FAULT_NOBITS)
      SAYF(cGRA " name = %s,   len = %u, map size = %u, exec speed = %llu us\n" cRST, 
